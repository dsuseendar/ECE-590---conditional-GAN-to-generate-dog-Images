...
def prepare_parser():
  ...
  parser.add_argument(
    '--base_root', type=str, default='/data/biggan',
    help='Default location to store all metas,weights, samples, data, and logs '
           ' (default: %(default)s)')
  parser.add_argument(
    '--seed', type=int, default=0,
    help='Random seed to use.')
  parser.add_argument(
    '--random_crop', type=str, default=None)
  parser.add_argument(
    '--ratio_mode', type=int, default=0, help='0:Square 1:Keep origin ratio 2:blend')
...
def run(config):
  ...
  out_meta_dir=f'{config["base_root"]}/meta/'
  os.makedirs(out_meta_dir,exist_ok=True)
  out_fname=out_meta_dir+config['dataset'].strip('_hdf5')+'_inception_moments.npz'
  np.savez(out_fname, **{'mu' : mu, 'sigma' : sigma})
