...
def prepare_parser():
  ...
  parser.add_argument(
    '--add_blur', action='store_true', default=False,
  ...
  parser.add_argument(
    '--add_style', action='store_true', default=False,
  ...
  parser.add_argument(
    '--mix_style', action='store_true', default=False,
    help='use mix style? (default: %(default)s)')

  parser.add_argument(
    '--dim_dog_cnt_z', type=int, default=32,
    help='dog_cnt z dimensionality: %(default)s)')

  parser.add_argument(
    '--use_dog_cnt', action='store_true', default=False,
    help='use dog count in a image? (default: %(default)s)')
  ...
  parser.add_argument(
    ...
    help='Use a hash of the experiment name instead of the full config '
         '(default: %(default)s)')
  parser.add_argument(
    '--base_root', type=str, default='/data4/data/gan_dogs/biggan',
  ...
  parser.add_argument(
    '--on_kaggle', action='store_true', default=False)
  parser.add_argument(
    '--debug', action='store_true', default=False)
  parser.add_argument(
    '--crop_mode', type=int, default=0, help='0:origin '
                                            '1:max_crop '
                                            '2:max_crop*0.5 + origin*0.5'
                                            '3:max_crop*0.33 + origin*0.33 + CenterCropLongEdge*0.33'
                                            '4:max_crop + origin(-multi dogs -small dogs) + CenterCropLongEdge'
                                            '5:max_crop*0.4 + origin*0.2 + CenterCropLongEdge*0.4'
                                            '6:max_crop*0.4 + origin*0.4 + CenterCropLongEdge*0.2'
                                            '7:max_crop*0.2 + origin*0.4 + CenterCropLongEdge*0.4'
                                            '8:max_crop*0.5 + CenterCropLongEdge*0.5')
  parser.add_argument(
  ...
  parser.add_argument(
    '--without_multi_dogs', action='store_true', default=False)
  return parser
...
dset_dict = {'I32': dset.ImageFolder, 'I64': dset.ImageFolder,
             ...
             'Dog128': dset.DogDataset,'Dog96': dset.DogDataset,'Dog64': dset.DogDataset,'Dog32': dset.DogDataset,
             'DogOrigin64': dset.ImageFolder,'DogOrigin': dset.ImageFolder,
             'DogOrigin96': dset.ImageFolder,
             'DogOrigin128': dset.ImageFolder,
             'DogOriginSquare96': dset.DogDataset,
             'DogOriginSquare80': dset.DogDataset,
             'DogOriginMixCR96': dset.DogDataset,
             'DogOriginMixCR80': dset.DogDataset,
             }
...
def get_data_loaders(dataset, data_root=None, augment=False, batch_size=64,
  ...
  if dataset.startswith('Dog'):
    ...
    dataset_kwargs['use_dog_cnt'] = kwargs.get('use_dog_cnt', False)
    dataset_kwargs['n_classes'] = kwargs.get('n_classes', 120)
    dataset_kwargs['without_multi_dogs'] = kwargs.get('without_multi_dogs', False)
  ...
  if 'hdf5' in dataset:
    ...
    if augment == 1:
      ...
      if dataset in ['C10', 'C100']:
        ...
        resize_trans1=transforms.Compose([RandomCropLongEdge(), transforms.Resize(image_size, kwargs['resize_mode'])])
      ...
      if dataset in ['C10', 'C100']:
        ...
        train_transform = [CenterCropLongEdge(), transforms.Resize(image_size, kwargs['resize_mode'])]
    else:
...
def sample(G, z_, y_, config):
  with torch.no_grad():
    ...
    if config['use_dog_cnt'] and config['G_shared']:
      ye0 = G.shared(y_[:, 0])
      ye1 = G.dog_cnt_shared(y_[:, 1])
      gyc = torch.cat([ye0, ye1], 1)
    else:
      gyc = G.shared(y_)
    if config['parallel']:
      G_z =  nn.parallel.data_parallel(G, (z_, gyc))
    else:
      G_z = G(z_, gyc)
    return G_z, y_
...
def sample_sheet(G, classes_per_sheet, num_classes, samples_per_class, parallel,
                 samples_root, experiment_name, folder_number, z_=None,G_shared=False,use_dog_cnt=False):
  ...
  if use_dog_cnt:
    cnt = np.array([19221, 1205, 122, 31])
    dog_cnt_probs = cnt / cnt.sum()

  # loop over total number of sheets
  for i in range(num_classes // classes_per_sheet):
    ims = []
    if use_dog_cnt:
      y0 = torch.arange(i * classes_per_sheet, (i + 1) * classes_per_sheet, device='cuda')
      num_dogs = np.random.choice(np.arange(len(dog_cnt_probs)), classes_per_sheet, p=dog_cnt_probs)
      num_dogs = torch.from_numpy(num_dogs).cuda()
      y=torch.cat([y0.unsqueeze(1),num_dogs.unsqueeze(1)], dim=1)
    else:
    ...
    for j in range(samples_per_class):
      if (z_ is not None) and hasattr(z_, 'sample_') and classes_per_sheet <= z_.size(0):
        ...
        z_ = torch.randn(classes_per_sheet, z_.size(1), device='cuda')
      with torch.no_grad():

        # Get Generator output given noise
        if use_dog_cnt and G_shared:
          ye0 = G.shared(y[:, 0])
          ye1 = G.dog_cnt_shared(y[:, 1])
          gyc = torch.cat([ye0, ye1], 1)
        else:
          gyc=G.shared(y)

        if parallel:
          o = nn.parallel.data_parallel(G, (z_[:classes_per_sheet], gyc))
        else:
...
#             if not os.path.isdir('%s/%s/%d/%d' % (samples_root, experiment_name, folder_number,y[j])):
#                 os.mkdir('%s/%s/%d/%d' % (samples_root, experiment_name, folder_number,y[j]))
            image_filename = '%s/%s/%d/%s__%d.jpg' % (samples_root, experiment_name,
                                                 folder_number, classes[j], i)
            imgtosave = ims[i][j,:,:,:].view(-1, ims[0].shape[1], ims[0].shape[2],
                                       ims[0].shape[3]).data.float().cpu()
            torchvision.utils.save_image(imgtosave, image_filename, normalize=True)
...
def interp_sheet(G, num_per_sheet, num_midpoints, num_classes, parallel,
  ...
  dim_z=G.dim_z*2 if G.mix_style else G.dim_z
  # Prepare zs and ys
  if fix_z: # If fix Z, only sample 1 z per row
    zs = torch.randn(num_per_sheet, 1, dim_z, device=device)
    zs = zs.repeat(1, num_midpoints + 2, 1).view(-1, dim_z)
  else:
...
class Distribution(torch.Tensor):
  ...
  def init_distribution(self, dist_type, **kwargs):
    ...
    elif self.dist_type == 'categorical_dog_cnt':
      self.num_categories = kwargs['num_categories']
      cnt=np.array([19221, 1205, 122, 31])
      self.dog_cnt_probs = cnt/cnt.sum()
  ...
  def sample_(self):
    if self.dist_type == 'normal':
      ...
      #perhaps there is a problem of performance since the data transfer to GPU
      batch_size=len(self)
      categories = np.random.choice(np.arange(self.num_categories), batch_size,replace=True)
      categories = torch.from_numpy(categories)
      num_dogs = np.random.choice(np.arange(len(self.dog_cnt_probs)), batch_size, p=self.dog_cnt_probs)
      num_dogs = torch.from_numpy(num_dogs)
      v = torch.cat([categories.unsqueeze(1),num_dogs.unsqueeze(1)], dim=1)
      self.set_(v.long().cuda())
    elif self.dist_type=='censored_normal':
      self.normal_(self.mean, self.var)
      self.relu_()
    elif self.dist_type=='bernoulli':
...
def prepare_z_y(G_batch_size, dim_z, nclasses, device='cuda',
                fp16=False,z_var=1.0,z_dist='normal', threshold=1,y_dist='categorical'):
  ...
  if y_dist=='categorical':
    ...
    y_.init_distribution(y_dist, num_categories=nclasses)
    y_ = y_.to(device, torch.int64)
  elif y_dist=='categorical_dog_cnt':
    y_ = Distribution(torch.zeros([G_batch_size,2], requires_grad=False))
    y_.init_distribution(y_dist, num_categories=nclasses)
    y_ = y_.to(device, torch.int64)


  # y_cnt_ = Distribution(torch.zeros(G_batch_size, requires_grad=False))
  # y_cnt_.init_distribution('dog_cnt', num_categories=4)
  # y_cnt_ = y_cnt_.to(device, torch.int64)
